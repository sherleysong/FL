...
# [0 - fixed_size ， fixed_size  - fixed_size *2, ......]
def split_client_datasetFixed(num_clients: int, len_dataset: int, fixed_size: int = None):
    ...
    for client in range(num_clients):
        data_list = np.arange(fixed_size * client, fixed_size * client + fixed_size - 6)
        client_data_list.append(data_list)
        print(data_list)
    return client_data_list

# [0 -len_dataset]
def split_client_datasetFixedTotal(num_clients: int, len_dataset: int, fixed_size: int = None):
    client_data_list = []
    for client in range(num_clients):
        data_list = np.arange(0, len_dataset - 1)
        client_data_list.append(data_list)
        print(data_list)
    return client_data_list

# per 10 minutes 1 data .
# non-iid - 1 : 0~35 data (0 ~ 6  o'clock)  for clientA, 36 ~71(6-12 o'clock ) for clientB , 12-18 for clientC, 18-24 for clientD
def split_client_datasetnoniid(num_clients: int, len_dataset: int):
    """ 以每 144 个数据为一组，len_dataset， 其中0～35条数据（0～6点）给client1，第二个36～71（6-12点）给client2......
      Args:
          num_clients: number of clients
          len_dataset: Number of samples in the whole dataset
      Returns:
    ...
    """
    client_data_list = []
    lenthOfDay = int(len_dataset / 144)

    for client in range(num_clients):
        ind = int(client % 4)
        item = []
        for i in range(lenthOfDay):
            startnum = 144 * i + 36 * ind
            # 0 ~ 36, 144~144+36, 288 ~ 288+36
            data_list = np.arange(startnum, startnum + 36)
...
# non-iid - 2 : 2-6 o'clock for clientsABC, others for clientD
def split_client_datasetnoniid2(num_clients: int, len_dataset: int):
    """ 以每 144 个数据为一组，len_dataset， 其中0～35条数据（1～6点）给client1，第二个36～71（6-12点）给client2......
      Args:
          num_clients: number of clients
          len_dataset: Number of samples in the whole dataset
      Returns:
