...
for round in range(t_round):
    ...
    for c in range(num_clients):
        ...
        paramsx.append([param_after, mse, r2])

        acc_list.append([round + 1, score[0], score[1], mse, r2])

    # 已知第一个客户端数据是iid的，给40%的权重，其他non-iid的，给0.2的权重
    averageR2 = []
    for para in range(len(paramsx)):
        averageR2.append(paramsx[para][2])
    print(averageR2)
    averageR2v = average(averageR2)
    print(averageR2v)

    for para in range(len(paramsx)):
        if(paramsx[para][2] > averageR2v ):
            weight_acc += param_after * 0.5
